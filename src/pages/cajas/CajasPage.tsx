...
import type { CajaFormData } from "./components/CajaForm";
...
const CajasPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useGetAllCajasQuery({
    page,
    query: {
  ...
  const { data: selectedCaja, isLoading: isLoadingCaja } = useGetCajaQuery(
    (modalState.id as number) || 0,
    {
      skip: modalState.type !== "edit" || !modalState.id,
    }
  );

  const [addCaja, { isLoading: isAdding }] = useAddCajaMutation();
  const [updateCaja, { isLoading: isUpdating }] = useUpdateCajaMutation();
  const [deleteCaja, { isLoading: isDeleting }] = useDeleteCajaMutation();
  const [addMontoCaja, { isLoading: isAddingMonto }] =
    useAddMontoCajaMutation();
  const [subtractMontoCaja, { isLoading: isSubtractingMonto }] =
    useSubtractMontoCajaMutation();

  const handleAdd = async (formData: CajaFormData) => {
    try {
      await addCaja(formData).unwrap();
      notify.success("Caja creada exitosamente");
      closeModal();
    } catch (err: any) {
      notify.error(err?.data?.message || "Error al crear la caja");
    }
  };

  const handleEdit = async (formData: CajaFormData) => {
    ...
    try {
      ...
      notify.error(err?.data?.message || "Error al actualizar la caja");
    }
  };
  ...
  const handleDelete = async () => {
    if (!modalState.id) return;
    try {
      await deleteCaja(modalState.id as number).unwrap();
      notify.success("Caja eliminada exitosamente");
      closeModal();
    } catch (err: any) {
      notify.error(err?.data?.message || "Error al eliminar la caja");
    }
  };
  ...
  const columns: ColumnConfig<CajaDto>[] = [
    {
      key: "nombre",
      header: "Nombre",
    },
  ...
  const actions: ActionMenuItem<CajaDto>[] = [
    {
      label: "Agregar Monto",
  ...
  return (
    <Box>
      ...
      <FormDrawer
        ...
        title="Nueva Caja"
      >
        <CajaForm onSubmit={handleAdd} loading={isAdding} />
      </FormDrawer>
      ...
      >
        {isLoadingCaja ? (
          ...
          <CajaForm
            ...
            defaultValues={
              selectedCaja
                ? { nombre: selectedCaja.nombre, localId: selectedCaja.localId }
                : undefined
            }
            loading={isUpdating}
          />
      ...
      <ConfirmDialog
        ...
        title="Eliminar Caja"
        description={`¿Está seguro que desea eliminar la caja "${modalState.data?.nombre}"? Esta acción no se puede deshacer.`}
      ...
      >
        <CajaDetails caja={selectedCaja} loading={isLoadingCaja} />
      </FormDrawer>
