...
import type { OperadoraFormData } from "./components/OperadoraForm";
import { OperadoraDetails } from "./components/OperadoraDetails";
...
const OperadorasPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useGetAllOperadorasQuery({
    page,
    query: {
      nombre: appliedFilters.nombre || undefined,
      numeroTelefono: appliedFilters.numeroTelefono || undefined,
      localId: selectedLocalId || appliedFilters.localId?.value || undefined,
    },
  });

  const paginatedData = useMemo(() => {
    if (!data?.data) return [] as OperadoraDto[];
    return data.data.slice(0, pageSize);
  }, [data, pageSize]);

  const { data: selectedOperadora, isLoading: isLoadingOperadora } =
    useGetOperadoraQuery((modalState.id as number) || 0, {
      skip:
        !modalState.id ||
        (modalState.type !== "edit" && modalState.type !== "details"),
    });

  const [addOperadora, { isLoading: isAdding }] = useAddOperadoraMutation();
  const [updateOperadora, { isLoading: isUpdating }] =
    useUpdateOperadoraMutation();
  const [deleteOperadora, { isLoading: isDeleting }] =
    useDeleteOperadoraMutation();
  const [addMontoOperadora, { isLoading: isAddingMonto }] =
    useAddMontoOperadoraMutation();
  const [subtractMontoOperadora, { isLoading: isSubtractingMonto }] =
    useSubtractMontoOperadoraMutation();

  const handleAdd = async (formData: OperadoraFormData) => {
    try {
      await addOperadora(formData).unwrap();
      notify.success("Operadora creada exitosamente");
      closeModal();
    } catch (err: any) {
      notify.error(err?.data?.message || "Error al crear la operadora");
    }
  };

  const handleEdit = async (formData: OperadoraFormData) => {
    if (!modalState.id) return;
    try {
      await updateOperadora({
        id: modalState.id as number,
        body: formData,
      }).unwrap();
      notify.success("Operadora actualizada exitosamente");
      closeModal();
    } catch (err: any) {
      notify.error(err?.data?.message || "Error al actualizar la operadora");
    }
  };
  ...
  const handleDelete = async () => {
    if (!modalState.id) return;
    try {
      await deleteOperadora(modalState.id as number).unwrap();
      notify.success("Operadora eliminada exitosamente");
      closeModal();
    } catch (err: any) {
      notify.error(err?.data?.message || "Error al eliminar la operadora");
    }
  };
  ...
  const columns: ColumnConfig<OperadoraDto>[] = [
    {
      key: "nombre",
      header: "Nombre",
    },
    {
      ...
      render: (row) => (
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="body2">{row.numeroTelefono}</Typography>
        </Box>
      ),
    },
    {
      key: "montoBilletera",
      header: "Monto (Gs.)",
      render: (row) => formatCurrency(row.montoBilletera),
    },
    {
      key: "localNombre",
      header: "Local",
  ...
  const actions: ActionMenuItem<OperadoraDto>[] = [
    {
      label: "Agregar Monto",
  ...
  return (
    <Box>
      ...
      <FormDrawer
        ...
        onClose={closeModal}
        title="Nueva Operadora"
      >
        <OperadoraForm onSubmit={handleAdd} loading={isAdding} />
      </FormDrawer>
        ...
        {isLoadingOperadora ? (
          ...
          <OperadoraForm
            ...
            defaultValues={
              selectedOperadora
                ? {
                    nombre: selectedOperadora.nombre,
                    localId: selectedOperadora.localId,
                    numeroTelefono: selectedOperadora.numeroTelefono,
                  }
                : undefined
            }
            loading={isUpdating}
      ...
      <ConfirmDialog
        ...
        description={`¿Está seguro que desea eliminar la operadora "${modalState.data?.nombre}"? Esta acción no se puede deshacer.`}
      ...
      <FormDrawer
        ...
        onClose={closeModal}
        title="Detalles de Operadora"
      >
        <OperadoraDetails
          operadora={selectedOperadora}
          loading={isLoadingOperadora}
        />
      </FormDrawer>
